...
{
    //decimos cuanto va a durar la animación
    ...
    public Vector3 finalEulerRotation = new Vector3(0f, 0f, 180f); //z en 180 para que queden boca abajo por como está hecho el prefab

    private bool isAnimating = false; //Contolador para evitar que dos animaciones se hagan al mismo tiempo

    public void AnimateCard(Transform startPos, Transform endPos) //Vamos a recoger dos variables, una posición inicial y otra posición final, éstas serán el componenente transform de algún objeto
        //haciendo que la carta vaya desde la posición del primer objeto con transform (startPos) al segundo, este método es público
    {
        ...
        {
            StartCoroutine(AnimateMovement(startPos.position, endPos.position, Quaternion.Euler(finalEulerRotation)));//Le damos a la corrutina el componente position de los transform de los objetos , además vamos a acceder
    ...
    private IEnumerator AnimateMovement(Vector3 from, Vector3 to, Quaternion finalRotation) //Recogemos el componente position que nos está pasando la función AnimateCard de startPos y le llamamos from
        //hacemos lo mismo con endPos y lo llamamos to, y la rotación como finalRotation, éste método es privado
    {
        ...
        while (time < moveDuration)//Aquí empezamos a animar el objetomoviendolo y rotandolo, una vez se termine el tiempo estipulado el while parará de actuar
        {
            float t = time / moveDuration;// t es el tiempo normalizado, su valor estará entre el 0 y el 1
